...
//plain data snapshot passed into the state machine each loop, driver modules (IMU, baro, GPS, etc.) fill this; the state machine does not know how to talk to any sensor directly.
struct SensorData {
    //IMU
    float accel_x_g;
    ...
    float accel_z_g;
    bool imu_valid;
    //barometers, onboard is primary, nosecone is secondary
    ...
    bool pyro_continuity[pyroNumber];//pyro continuity (per channel, true = continuity present)
...
struct FlightCommands {
    bool fire_pyro[pyroNumber];//edge-triggered fire request
...
private:
    ...
    bool pyroFireLatched[pyroNumber];
    //per-state handlers
